...
Tested on: FortiGate 7.4.7, 7.4.8, 7.4.9, 7.4.10, 7.4.11,7.6.2,7.6.1,
           FortiAnalyzer 7.6.6M  (x64 & aarch64)

## Python decrypt scripts (newer firmware)

Standalone Python scripts for firmware versions that use the dynamic RSA+AES scheme:

| Script | Versions | ChaCha20 rot | Payload | Notes |
|--------|----------|-------------|---------|-------|
| `fgt747_decrypt.py` | 7.4.7, 7.4.8, 7.4.9, 7.4.10, 7.4.11 | 5/2 | 48 B (IV\|KEY) | Seed VA varies per build |
| `fgt762_decrypt.py` | 7.6.2 | 6/3 | 80 B (??\|CTR\|KEY) | Seed VA as 7.6.1 |
| `fgt761_decrypt.py` | 7.6.1 | 6/3 | 80 B (??\|CTR\|KEY) | Same VA as 7.6.2 |
| `faz766m_decrypt.py` | FortiAnalyzer 7.6.6M | AES-128-CTR (crypto\_inc, big-endian) | — | Hardcoded key, no RSA |

### Usage (7.4.x / 7.6.x FGT)

```bash
# Requires: pip install pyelftools pycryptodome vmlinux-to-elf
vmlinux-to-elf flatkc flatkc.elf
python3 fgt747_decrypt.py decrypt flatkc.elf rootfs.gz rootfs.gz.dec   # 7.4.x
python3 fgt761_decrypt.py decrypt flatkc.elf rootfs.gz rootfs.gz.dec   # 7.6.1
python3 fgt762_decrypt.py decrypt flatkc.elf rootfs.gz rootfs.gz.dec   # 7.6.2
gunzip -c rootfs.gz.dec | cpio -id   # extract filesystem
```

### Usage (FAZ 7.6.6M)

```bash
python3 faz766m_decrypt.py decrypt rootfs.gz rootfs.dec
xz -d -c rootfs.dec | cpio -id
```
